using System;
using System.Text;

class Encrypt
{
    //Write a program that encodes and decodes a string using given encryption key (cipher). 
    //The key consists of a sequence of characters. The encoding/decoding is done by performing
    //XOR (exclusive or) operation over the first letter of the string with the first of the key, 
    //the second – with the second, etc. When the last key character is reached, the next is the first.

    static string EncryptText(string text, string cipher)
    {

        StringBuilder encryptedText = new StringBuilder();

        for (int i = 0; i < text.Length; i++)
        {
            encryptedText.Append((char)(text[i] ^ cipher[i % cipher.Length]));

        }
        return encryptedText.ToString();
    }


    static void Main()
    {
        Console.Write("Write a cipher: ");
        string cipher = Console.ReadLine();
        Console.Write("Text to be encrypted: ");
        string text = Console.ReadLine();
        string encryptedText = EncryptText(text, cipher);
        Console.WriteLine("After encoding: " + encryptedText);
        Console.WriteLine("After decoding: " + EncryptText(encryptedText, cipher));
    }
}
